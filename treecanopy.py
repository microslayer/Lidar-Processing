...
'''data = gdal.Open(dstfile)

print data.RasterCount

treeband = data.GetRasterBand(1)
treebandarray = treeband.ReadAsArray()
outputshp = "PolygonTest4"
drv = ogr.GetDriverByName("ESRI Shapefile")

newoutds = drv.CreateDataSource("H:/Lidar/"+outputshp + ".shp")
newoutlayer = newoutds.CreateLayer(outputshp, srs = None)

newField = ogr.FieldDefn('MYFLD', ogr.OFTInteger)
newoutlayer.CreateField(newField)

gdal.Polygonize(treeband, None, newoutlayer, -1, [], callback=None)

data.FlushCache()
data = None'''

def makepolygon(input_file,outputfile):
